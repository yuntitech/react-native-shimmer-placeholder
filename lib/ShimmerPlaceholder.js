...
class ShimmerPlaceHolder extends Component {
  ...
  render() {
    ...
    const lgStart = linearGradientStart ?? { x: -1, y: 0.5 };
    const lgEnd = linearGradientEnd ?? { x: 2, y: 0.5 };
    return (
      <View
...
ShimmerPlaceHolder.propTypes = {
  ...
  linearGradientStart: PropTypes.oneOfType({x: PropTypes.number, y: PropTypes.number}),
  linearGradientEnd: PropTypes.oneOfType({x: PropTypes.number, y: PropTypes.number})
};
